...
class ProductService {
	public getProducts(start: number = 0, offset?: number): Promise<Product[]> {
		return new Promise((resolve) => {
			setTimeout(() => {
				const end = offset ? start + offset : productsData.length;
				resolve(productsData.slice(start, end));
			}, 1000);
		});
	}

	public getProductByKey<T extends keyof Product>(key: T, value: Product[T]): Promise<Product> {
		return new Promise((resolve) => {
			setTimeout(() => {
				resolve(productsData.find((product) => product[key] === value));
			}, 1000);
		});
	}
	...
	public getAllProductsByKey<T extends keyof Product>(key: T, value: Product[T]): Promise<Product[]> {
		return new Promise((resolve) => {
			setTimeout(() => {
				resolve(productsData.filter((product) => product[key] === value));
			}, 1000);
		});
	}
}

// todo-> реализовать DI и упрощенную версию IoC контейнера
